...
test("the lab stays same-origin while API traffic is transparently rewritten", async () => {
    ...
    assert.match(
        config,
        /destination:\s*`\$\{ampLabelLabApiOrigin\}\/api\/:path\*`/,
    );
    ...
    assert.match(config, /process\.env\.AMP_LABEL_LAB_API_ORIGIN/);
